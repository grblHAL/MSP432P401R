...
static inline __attribute__((always_inline)) uint8_t get_idx (input_signal_t *input)
{
    uint_fast8_t port = n_in;
    do {
        port--;
        if(input->port == aux_in[port].port && input->pin == aux_in[port].pin)
            break;
    } while(port);
...
void ioports_event (input_signal_t *input)
{
    ...
    if(input->interrupt_callback)
        input->interrupt_callback(in_map_rev(get_idx(input)), BITBAND_PERI(input->port->IN, input->pin));
    else
        event_bits |= input->bit;
