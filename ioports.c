    ...
    if(invert.mask != settings.ioport.invert_out.mask) {
        ...
        do {
            ...
            if(((settings.ioport.invert_out.mask >> port) & 0x01) != ((invert.mask >> port) & 0x01))
                BITBAND_PERI(aux_out[port].port->OUT, aux_out[port].pin) = !BITBAND_PERI(aux_out[port].port->IN, aux_out[port].pin);
        } while(port);
...
//    aux_set_pullup();
    ...
    if(n_out) do {
        idx--;
        BITBAND_PERI(aux_out[idx].port->OUT, aux_out[idx].pin) = (settings.ioport.invert_out.mask >> idx) & 0x01;
    } while(idx);
}
...
static void aux_set_pullup (void)
{
    ...
    GPIO_Init.Pull = settings.ioport.pullup_disable_in.pin0 ? GPIO_PULLDOWN : GPIO_PULLUP;
    HAL_GPIO_Init(AUXINPUT0_PORT, &GPIO_Init);

    GPIO_Init.Pin = AUXINPUT1_PIN;
    GPIO_Init.Pull = settings.ioport.pullup_disable_in.pin1 ? GPIO_PULLDOWN : GPIO_PULLUP;
...
{
    if(port < n_out)
        BITBAND_PERI(aux_out[port].port->OUT, aux_out[port].pin) = ((settings.ioport.invert_out.mask >> port) & 0x01) ? !on : on;
}
...
inline static __attribute__((always_inline)) int32_t get_input (const input_signal_t *input, bool invert, wait_mode_t wait_mode, float timeout)
{
    if(wait_mode == WaitMode_Immediate)
        return BITBAND_PERI(input->port->IN, input->pin) ^ invert;
    ...
    if(wait_mode == WaitMode_Rise || wait_mode == WaitMode_Fall) {
        ...
        if(input->cap.irq_mode & irq_mode) {
            ...
            do {
                if(event_bits & input->bit) {
                    value = BITBAND_PERI(input->port->IN, input->pin) ^ invert;
                    break;
                }
                if(delay) {
        ...
        do {
            if((BITBAND_PERI(input->port->IN, input->pin) ^ invert) == wait_for) {
                value = BITBAND_PERI(input->port->IN, input->pin) ^ invert;
                break;
            }
            if(delay) {
...
static inline uint8_t get_idx(input_signal_t *input)
{
    uint_fast8_t idx = n_in;
    do {
        idx--;
        if(input->port == aux_in[idx].port && input->pin == aux_in[idx].pin)
            break;
    } while(idx);
...
void ioports_event (input_signal_t *input)
{
    ...
    if(input->interrupt_callback)
        input->interrupt_callback(get_idx(input), BITBAND_PERI(input->port->IN, input->pin));
    else
        event_bits |= input->bit;
...
xbar_t *get_pin_info(bool digital, uint8_t port)
{
    static xbar_t info;

    if(digital) {
        info.pin = aux_in[port].pin;
